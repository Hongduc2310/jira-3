...
interface IProjectTaskCol {
    taskDetailList: ITaskDetailList;
    idx: number;
    renderProjectCard: (taskDetailList: ITask[]) => React.ReactNode;
}

const ProjectTaskStatusCol = (
    { taskDetailList, idx, renderProjectCard }: IProjectTaskCol) => {
    return (
        <Droppable key={taskDetailList.statusId.toString() + idx} droppableId={taskDetailList.statusId.toString()}>
            {
                (provided) => {
                    return (
                        <div
